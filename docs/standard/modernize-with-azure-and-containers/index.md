...
## <a name="path-to-the-cloud-for-existing-net-applications"></a>기존.NET 응용 프로그램을 클라우드로 옮기는 방법
...
그림 1-1에서는 점진적으로 기존.NET 응용 프로그램을 클라우드로 옮기는 경우 선택할 수 있는 방법을 보여 줍니다.

 ![기존 .NET 응용 프로그램과 서비스을 최신으로 변경하는 방법](./media/image1-1.png)

> **그림 1-1**. 기존 .NET 응용 프로그램과 서비스를 최신으로 변경하는 방법

각 마이그레이션 방법은 다양한 이점과 사용하는 이유가 있습니다. 클라우드에 앱을 마이그레이션 할 때 하나의 방법을 선택하거나 여러 방법 중에서 특정 구성 요소를 선택할 수 있습니다. 개별 응용 프로그램은 하나의 접근 방법이나 성숙도 상태로 제한되지 않습니다. 예를 들면 보통의 하이브리드 방식에서는 특정 온-프레미스 구성 요소와 그 밖의 클라우드 구성 요소를 사용합니다.

다음은 각 응용 프로그램의 성숙도에 대한 간략한 설명과 정의입니다.

**수준 1: 클라우드 인프라를 갖춘** 응용 프로그램:이 마이그레이션 방식은 현재의 온-프레미스 응용 프로그램을 서비스 형태의 인프라([IaaS](https://azure.microsoft.com/overview/what-is-iaas/)) 플랫폼으로 간단히 마이그레이션하거나 호스트를 변경합니다. 앱의 구성은 전과 거의 동일하지만 이제 클라우드의 VM에 앱을 배포합니다.
이 간단한 마이그레이션 유형을 보통 업계에서는 "리프트 엔 시프트"라고 합니다.

**수준 2: 클라우드 최적화** 응용 프로그램:이 수준에서는 여전히 재설계나 중대한 코드 변경을 하지 않아도 추가적인 클라우드 관리 서비스와 컨테이너 같은 최신 기술 사용 하여 클라우드에서 앱을 실행하여 추가적인 이점을 얻을 수 있습니다. 기업 개발 운영(DevOps) 프로세스를 세분화하여 응용 프로그램의 민첩성을 개선함으로써 더 빨리 출시할 수 있습니다. Docker 엔진을 기반으로 하는 Windows 컨테이너 같은 기술을 사용하여 이 작업을 수행 합니다. 컨테이너는 여러 단계로 배포할 때 응용 프로그램 종속성에서 발생하는 마찰을 제거 합니다. 이 성숙도 모델에서는 IaaS의 컨테이너를 배포하거나, 모니터링과 지속적인 통합/지속적인 배포 (CI/CD) 파이프라인, 서비스 형태의 캐시, 데이터베이스와 관련한 추가적인 클라우드 관리 서비스를 사용하는 PaaS에 배포할 수 있습니다.

세 번째 성숙도는 클라우드의 궁극적 목표지만, 여러 앱에는 선택사항이며 이 가이드에서 중요하게 다루는 부분은 아닙니다.

**수준 3: 클라우드-네이티브** 응용 프로그램: 일반적으로서 이 마이그레이션 방법은 비즈니스 필요성과 업무상 중요한 응용 프로그램을 최신으로 만들기 위한 목적으로 결정합니다. 이 수준에서는 PaaS 서비스를 사용하여 PaaS 컴퓨팅 플랫폼으로 앱을 옮깁니다. [클라우드 전용](https://www.gartner.com/doc/3181919/architect-design-cloudnative-applications) 응용 프로그램과 마이크로 서비스 아키텍처를 구현하여 장기적으로 응용 프로그램의 민첩성을 개선하고 새로운 한계로 확장합니다. 이 유형으로 최신화하기 위해서는 일반적으로 클라우드를 위한 특별한 설계가 필요합니다. 특히 클라우드 전용 응용 프로그램과 마이크로 서비스 기반 모델로 변경하는 경우 새로운 코드를 작성해야 하는 경우가 많습니다. 이 방법은 모놀리식과 온-프레미스 응용 프로그램 환경에서는 기대하기 어려웠던 이점을 얻을 수 있습니다.

표 1-1에서는 각각의 마이그레이션이나 최신화 방법을 선택하는 이유와 주요 이점을 설명합니다.

| **클라우드 인프라 지원** <br /> *리프트 앤 시프트* | **클라우드 최적화** <br /> *최신화* | **클라우드 네이티브** <br /> *최신화와 재설계, 재작성* |
|---|---|---|
| **응용 프로그램의 계산 대상** |
| Azure의 VM에 배포된 응용 프로그램 | Azure Service Fabric과 AKS (Azure Kubernetes Service), 컨테이너를 사용하여 Azure App Service, Azure 컨테이너 인스턴스 (ACI), VM에 배포된 모놀리식 또는 N-티어 앱| AKS(Azure Kubernetes Service)와 Service Fabric 또는 Azure 함수 기반의 서버리스 마이크로 서비스에서 컨테이너화된 마이크로 서비스. |
| **데이터 대상** |
...
- **클라우드 네이티브**:에 도입 된 설계 생각해 야 [클라우드 네이티브](https://www.gartner.com/doc/3181919/architect-design-cloudnative-applications) 작업을 수행 하는 여러 독립 개발 팀을 사용 하 여 크고 복잡 한 응용 프로그램을 대상으로 할 때 응용 프로그램 다른 마이크로 서비스를 개발 하 고 자율적으로 배포할 수 있습니다. 또한 마이크로 서비스 마다 granularized와 확장성 인해 이러한 아키텍처 접근 방법을 매우 중요 한 과제와 복잡 한 직면 하 고 있지만 클라우드 PaaS를 사용 하 여 크게 간소화할 수 있습니다 및 오 케 스트레이 터 [Azure Kubernetes Service (AKS/ACS)](https://azure.microsoft.com/services/container-service/) (managed Kubernetes), [Azure 서비스 패브릭 및 [Azure Functions](https://azure.microsoft.com/services/functions/) 서버 리스 방식에 대 한 합니다. 클라우드 용 설계 하 고 새 코드를 작성 하는 (예: 마이크로 서비스 및 서버) 이러한 모든 접근이 방법은 일반적으로 필요-특정 PaaS 플랫폼에 적용 되는 코드 또는 마이크로 서비스 같은 특정 아키텍처에 부합 하는 코드입니다.
